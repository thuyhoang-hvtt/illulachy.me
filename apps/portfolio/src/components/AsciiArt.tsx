...
type AsciiArtProps = {
  ...
  animated?: boolean;
  /** Animation style */
  animationStyle?: "fade" | "typewriter" | "matrix" | "flicker" | "none";
  ...
  /** Pause the animation loop (e.g. when hidden) */
  paused?: boolean;
};
const MATRIX_CHARSET = "ﾊﾐﾋｰｳｼﾅﾓﾆｻﾜﾂｵﾘｱﾎﾃﾏｹﾒｴｶｷﾑﾕﾗｾﾈｽﾀﾇﾍ";

type FlickerCell = { alpha: number; life: number; maxLife: number; char: string };
...
export const AsciiArt: React.FC<AsciiArtProps> = ({
  ...
  const flickerMapRef = useRef<Map<number, FlickerCell>>(new Map());
  ...
  const drawCanvas = useCallback(
    (progress: number = 1, matrixProgress?: number, flickerMap?: Map<number, FlickerCell>) => {
      ...
      for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
          ...
          // Apply flicker overlay for active cells
          const flickerKey = y * cols + x;
          const flicker = flickerMap?.get(flickerKey);
          if (flicker) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (shouldShowStatic || hasAnimated || !shouldStartAnimation || animationStyle === "flicker") {
        drawCanvas(1);
        return;
      }
  ...
  // Continuous flicker loop — runs independently of the one-shot animation system
  useEffect(() => {
    if (!isLoaded || asciiData.length === 0 || animationStyle !== "flicker" || !shouldStartAnimation || paused) return;

    const rows = asciiData.length;
    const cols = asciiData[0]?.length || 0;
    const totalCells = rows * cols;
    // ~0.3% of cells spawn per frame
    const spawnPerFrame = Math.max(1, Math.ceil(totalCells * 0.001));
    const flickerMap = flickerMapRef.current;
    let rafId: number;

    const loop = () => {
      // Decay existing flicker cells
      for (const [key, cell] of flickerMap) {
        cell.life -= 1;
        cell.alpha = cell.life / cell.maxLife;
        if (cell.life <= 0) flickerMap.delete(key);
      }

      // Spawn new flicker cells
      for (let i = 0; i < spawnPerFrame; i++) {
        const key = Math.floor(Math.random() * totalCells);
        if (!flickerMap.has(key)) {
          const maxLife = 100 + Math.floor(Math.random() * 200); // 10–30 frames
          const charIdx = Math.floor(Math.random() * effectiveCharset.length);
          flickerMap.set(key, {
            alpha: 1,
            life: maxLife,
            maxLife,
            char: effectiveCharset[charIdx] || " ",
          });
        }
      }

      drawCanvas(1, undefined, flickerMap);
      rafId = requestAnimationFrame(loop);
    };

    rafId = requestAnimationFrame(loop);
    return () => cancelAnimationFrame(rafId);
  }, [isLoaded, asciiData, animationStyle, shouldStartAnimation, paused, effectiveCharset, drawCanvas]);
